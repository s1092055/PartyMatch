...
async function main() {
  ...
  const g5ConvId = await lockGroup(H3, g5.id)
  ...
  const g9ConvId = await lockGroup(H2, g9.id)
  ...
  await activateGroup(H2, g9.id, g9ConvId, 'KKBOX')
  ...
  const g13ConvId = await lockGroup(D4, g13.id)
  for (const u of [H1, D5]) {
    const memberId = await getMemberId(D4, g13.id, u.id)
    await fillInfo(u, memberId, { email: u.email })
  }
  await activateGroup(D4, g13.id, g13ConvId, 'Cursor')
  ...
  const g18ConvId = await lockGroup(H2, g18.id)
  for (const u of g18Members) {
    const memberId = await getMemberId(H2, g18.id, u.id)
    await fillInfo(u, memberId, { email: u.email })
  }
  await activateGroup(H2, g18.id, g18ConvId, 'Microsoft 365')
  ...
  const g19ConvId = await lockGroup(H3, g19.id)
  for (const u of g19Members) {
    const memberId = await getMemberId(H3, g19.id, u.id)
    await fillInfo(u, memberId, { email: u.email })
  }
  await activateGroup(H3, g19.id, g19ConvId, 'Dropbox')
  await api('POST', `/groups/${g19.id}/dispute`, D2.token, { reason: '帳號被收回，登入不進去', evidenceUrl: 'https://picsum.photos/seed/dispute2/600/400' })
  ...
  await activateGroup(H1, g20.id, g20ConvId, 'NordVPN')
  await api('POST', `/groups/${g20.id}/dispute`, D5.token, { reason: '覺得速度跟描述不符', evidenceUrl: 'https://picsum.photos/seed/dispute3/600/400' })
  ...
  console.log(`\ndemo 資料建立完成！共 10 個帳號、22 個群組，demo1~demo9 密碼皆為: ${DEMO_PASSWORD}；demo-admin ${process.env.ADMIN_PASSWORD ? '使用 ADMIN_PASSWORD 環境變數設定的密碼' : `密碼同上（未設定 ADMIN_PASSWORD）: ${DEMO_PASSWORD}`}`)
