...
async function main() {
  console.log('清空資料中（保留 users、services，以及使用者註冊時收到的系統訊息與系統通知）...\n')
  ...
  await prisma.notification.deleteMany({ where: { type: { not: 'system' } } })
  console.log('  - notifications（system 類型的歡迎通知保留）')
  ...
  console.log('\n清空完成（users、services、系統訊息、系統通知已保留）')
}
