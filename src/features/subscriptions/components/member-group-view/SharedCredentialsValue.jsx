...
export function CredentialsValue({ group, viewerName, showPassword, onTogglePassword }) {
        ...
        <dl className="space-y-1 rounded-lg border border-line px-3 py-2.5">
          {parsedCredentials.map(({ key, label, value }) => (
            <div key={label} className="flex items-baseline justify-between gap-2 text-sm">
              ...
              {key === 'password' ? (
                <dd className="flex min-w-0 items-center gap-1.5 text-right text-brand dark:text-white">
                ...
                <dd className="min-w-0 truncate text-right text-brand dark:text-white">{value}</dd>
              )}
            </div>
          ))}
        </dl>
  ...
  if (group.sharedCredentials) {
    return (
      <CredentialWatermark viewerName={viewerName}>
        <p className="whitespace-pre-wrap rounded-lg border border-line px-3 py-2.5 text-sm text-brand dark:text-white">
...
export function MemberProvidedCredentialsValue({ serviceInfo, fields }) {
  const textFields = fields.filter(({ type }) => type !== 'checkbox')

  if (!serviceInfo || textFields.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-line px-3 py-2.5 text-sm text-ink-4">
        尚未填寫帳號資訊
      </p>
    ...
    <dl className="space-y-1 rounded-lg border border-line px-3 py-2.5">
      {textFields.map(({ key, label }) => (
        <div key={key} className="flex items-baseline justify-between gap-2 text-sm">
          <dt className="shrink-0 text-ink-4">{label}</dt>
          <dd className="min-w-0 truncate text-right text-brand dark:text-white">{serviceInfo[key] || '—'}</dd>
        </div>
      ))}
    </dl>
  )
