  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-line-subtle last:border-0">
      ...
      <div className="flex items-center gap-1 shrink-0 pt-5">
        {editing ? (
          <>
            <button onClick={save} aria-label="儲存" className="w-7 h-7 rounded-inner bg-brand hover:bg-brand-hover hover:-translate-y-0.5 flex items-center justify-center text-white transition-all"><Check size={13} strokeWidth={3} /></button>
            <button onClick={cancel} aria-label="取消編輯" className="w-7 h-7 rounded-inner border border-line hover:bg-raised hover:-translate-y-0.5 flex items-center justify-center text-ink-3 transition-all"><X size={13} /></button>
          </>
        ) : (
          <button onClick={() => setEditing(true)} aria-label={`編輯${label}`} className="w-7 h-7 rounded-inner border border-line hover:bg-raised hover:-translate-y-0.5 flex items-center justify-center text-ink-3 transition-all">
            <Pencil size={12} />
          </button>
        )}
      </div>
