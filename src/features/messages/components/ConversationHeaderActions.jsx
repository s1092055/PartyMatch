...
export default function ConversationHeaderActions({ selected, onMembersToggle }) {
  ...
  function handleViewGroup() {
    ...
    navigate(isHost ? '/my-groups?view=host' : '/my-groups?view=member', {
      state: { openGroupId: selected.groupId },
    })
    // HostPage 的跨頁面 effect 只在掛載當下讀 location.state，如果使用者本來就已經停在
    // /my-groups?view=host（訊息是全域掛載的浮層，隨時可能在任何頁面被打開），上面的 navigate
    // 不會觸發重新掛載；要靠這個事件走「同頁面」那條路徑才能真的把 modal 打開
    // （MemberPage 的等效 effect 是用 location.key 當 deps，navigate 到同頁也會重新觸發，不需要事件）
    if (isHost) {
