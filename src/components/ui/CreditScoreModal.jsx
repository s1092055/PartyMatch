    ...
    <>
        ...
        {showHistory ? (
          <>
            <div className="flex shrink-0 items-center gap-2 border-b border-line px-3 py-3">
              ...
              <span className="font-extrabold text-ink">信用分數紀錄</span>
            </div>
            {logsLoading ? (
      ...
      {!showHistory && hideFooter && (
        <div className="shrink-0 px-5 pb-4">
          <button
            onClick={() => setShowHistory(true)}
            className="flex w-full items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-medium text-ink-4 transition-colors hover:bg-raised hover:text-ink"
          >
            <Clock strokeWidth={1.5} size={13} /> 查看信用分數紀錄
          </button>
        </div>
