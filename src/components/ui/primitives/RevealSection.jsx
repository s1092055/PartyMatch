...
export default function RevealSection({ children, delay = 0, className = '' }) {
  ...
  // available 用 window.innerHeight（近似 100dvh）而不是 section.clientHeight，避免
  // 「縮小 → section 變矮 → available 變大 → 又放大」這種自我循環
  useLayoutEffect(() => {
    ...
    function recalc() {
      ...
      const dockReserve = dockVisible
        ? window.innerHeight - dockEl.getBoundingClientRect().top
        : 0
      const available = window.innerHeight - verticalPadding - dockReserve
