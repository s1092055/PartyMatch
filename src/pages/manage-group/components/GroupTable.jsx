...
export function GroupTable({ rows, title, description, emptyTitle, emptyMessage, emptyAction }) {
  ...
  return (
    <>
      <SectionPanel title={title} description={description} action={actionButtons}>
        {!rows.length ? (
          <EmptyState
            title={emptyTitle ?? "目前沒有資料"}
            description={emptyMessage ?? "目前尚未建立內容，可先到首頁加入或建立一個新群組。"}
            action={emptyAction}
          />
        ) : viewMode === "list" ? (
          <GroupListView
