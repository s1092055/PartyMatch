...
## 入口
- 側邊欄「續訂管理」按鈕（只有群組在啟用中狀態才會顯示），點擊開啟續訂管理視窗
...
| Model | 用途 |
|-------|------|
| `Group` | `status`、`nextBillingDate`、`billingCycle`、`escrowTokens`、`hasActivatedOnce`、`billingDateAdjustedAt`/`billingDateAdjustmentNote` |
| `Member` | 續訂時 `serviceInfo`/`serviceInfoIssueNote`/`confirmedAt` 全部清空，需要重新走一次填寫帳號流程 |
| `Subscription` | 續訂時 `nextBillingDate` 跟著同步更新，成員端卡片顯示的日期才不會停在上一期的舊值 |
...
- 前端算出的下一期帳單日跟逾期天數只是給彈窗顯示參考，實際帳單日還是以後端回傳的為準
- **下次扣款日的錨定與例外**：續訂時算出的下次扣款日是固定錨定值（上一期排定日期 + 一個週期），不受團主之後啟用服務的時間點影響——只有群組「第一次」啟用（尚未有任何一期成功啟用過）才會改成從實際啟用當下重新算一個週期，避免第一期因為鎖定到啟用中間拖延而縮水；之後每一期續訂啟用都固定沿用續訂當下算好的日期，不會再自動改動。團主若真的因為外部因素需要延後扣款日，走的是確認期的「調整下次扣款日」功能（每期限一次、最多延 7 天），不是靠啟用時間去間接影響
- 續訂當下算出的日期先以「預估」通知團主與所有成員（跟第一次鎖定群組同一套通知邏輯），並重置本期的扣款日調整額度，讓新的一期重新擁有一次可調整的機會

## 流程步驟

**1. 開啟續訂管理**
- 群組進入啟用中狀態後，側邊欄會顯示「續訂管理」，點擊開啟續訂管理視窗
- 視窗依帳單日算出「距帳單日還有 N 天」或「帳單日已過 N 天」，並列出兩個選項

**2. 開始新一期收款**
- 後端先查出所有成員的餘額，只要有人餘額不夠，就直接回錯誤（附上是哪些成員），不會進入交易
- 通過預檢後才真的扣款：用條件式更新嘗試扣全體成員，只要成功筆數跟成員數對不上（代表扣款當下餘額被其他請求變動），就整批回滾
- 扣款成功後批次寫入交易紀錄，清空所有成員的帳號資訊，群組狀態改回「等待填寫帳號資訊」，下次帳單日依繳費週期往後推一期，並重新設定 24 小時的填寫期限，跟第一次鎖定群組一樣會顯示倒數
...
## 驗證重點
- 只有團主本人能操作續訂，而且群組必須是啟用中狀態，其他狀態一律回錯誤並附上目前實際狀態
...
- 結束服務底層用的是通用的群組狀態更新，仍然受合法轉換規則限制，啟用中只能轉到確認期、已結束或等待確認幾個狀態，避免前端誤傳不合法的狀態值
