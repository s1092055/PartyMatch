...
- **群組卡片（`HostedGroupCard.jsx`，團主視角）依狀態切換統計格內容**：卡片頂部 badge 跟下方三個統計格（左／中／右）分別依 `group.status` 切換內容，中間格固定顯示「群組人數」，左／右格對照如下（成員視角的對應卡片邏輯較簡單，見「我的訂閱（成員視角）流程」文件）：
  ...
  | `pending_confirmation` 待成員填寫 | 成員填寫中 | 群組狀態：成員填寫中 | 群組人數 | 預估下次扣款（鎖定當下算出來的估計值，真正定案要等團主啟用服務） |
  ...
  「群組狀態」欄位文字（已滿員/成員填寫中/待啟用服務/確認期中/申訴中/已結束）跟頂部徽章是同一個狀態階段的兩種呈現，只有 `full`／`pending_confirmation` 兩階段徽章跟統計格文字不同步（徽章顯示「等待鎖定」／「成員填寫中」，統計格維持細分階段的原始文字）；待成員填寫這個階段代管費用其實已經在申請被接受當下扣完了（見 PM幣代管流程文件），不是在「收款」，因此左格用「群組狀態：成員填寫中」而非「收款紀錄」，跟這個階段實際在等待的事情（成員填寫服務帳號資訊）一致
