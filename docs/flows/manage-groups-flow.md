...
- **群組卡片（`HostedGroupCard.jsx`，團主視角）依狀態切換統計格內容**：卡片頂部 badge 跟下方三個統計格（左／中／右）分別依 `group.status` 切換內容，中間格固定顯示「成員人數」，左／右格對照如下（成員視角的對應卡片邏輯較簡單，見「我的訂閱（成員視角）流程」文件）：

  | `group.status` | 卡片頂部 Badge | 左格 | 中格 | 右格 |
  |---|---|---|---|---|
  | `recruiting` 招募中 | 招募中 | 待處理申請：{件數} | 成員人數 | 建立日期 |
  | `full` 額滿 | 等待鎖定（僅團主管理卡片顯示，其他地方的 `full` badge 仍是預設文字「已滿員」） | 群組狀態：已滿員 | 成員人數 | 下次扣款 |
  | `pending_confirmation` 待成員填寫 | 成員填寫中 | 群組狀態：成員填寫中 | 成員人數 | 預估下次扣款（鎖定當下算出來的估計值，真正定案要等團主啟用服務） |
  | `pending_activation` 待啟用 | 待啟用 | 群組狀態：待啟用服務 | 成員人數 | 預估下次扣款 |
  | `confirming` 確認期中 | 確認期中 | 群組狀態：確認期中 | 成員人數 | 下次扣款（已定案；`billingDateAdjustedAt` 為 null 時側邊欄另有「調整扣款日」按鈕） |
  | `disputed` 申訴中 | 問題處理中 | 群組狀態：問題處理中 | 成員人數 | 下次扣款 |
  | `active` 服務中 | 服務中（7 天內即將續訂時改顯示「即將續訂」） | 群組狀態：正常／追蹤中（收款未達全員時） | 成員人數 | 續訂日期 |
  | `cancelled` 已解散 | 已解散 | 收款紀錄：{件數} | 成員人數 | 建立日期 |
  | `ended` 已結束 | 已結束 | 收款紀錄：{件數} | 成員人數 | 建立日期 |
