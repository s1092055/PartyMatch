...
- 「選擇方案」內部（`md` 以上，桌機/平板寬度，跟高度無關）左右並排：左邊是切換箭頭在左右兩側、方案卡在中間（一次只顯示目前選中的方案，點左右箭頭切換上一個/下一個方案），右邊是方案內容（不再另外顯示「方案說明」標題）；右欄內容全部直接顯示、不再內部捲動，左右兩欄改用 flex row 預設的 `items-stretch` 自動等高（不用 `ResizeObserver` 量測），左欄上下加跟右側方案內容一樣的 `p-3.5` 內距（`py-3.5`），方案卡用 `self-stretch` 撐滿這一列扣掉內距後的高度，讓卡片邊框頂部對齊方案內容第一行文字、底部對齊最後一行文字（而不是直接對齊到整個欄位的最頂端/最底端），左右箭頭則維持固定大小、垂直置中；方案卡保留邊框（選中為淡化過的 `border-brand/40`，未選中為 `border-slate-200`，避免選中顏色太深）但不加背景色，內容維持垂直置中；手機（`md` 以下）則自然由上而下堆疊。步驟二、三現在都已經改回外層 `overflow-y-auto` 捲動整頁，不再有內部捲動區，`CreateGroupPage.jsx` 也已移除對應的 `short-lg:overflow-hidden`／`forwardWheel: false` 特殊處理
- 方案卡片標題不再重複顯示「（月繳）／（年繳）」字樣，計費週期改用卡片上方的獨立 badge 顯示（`currentPlan.billingCycle`），方案名稱因此變短；卡片本身（flex 項目）補上 `min-w-0`，讓 `truncate` 在名稱過長、卡片被壓縮時能正確截斷成「...」，不會撐開卡片、蓋到旁邊的切換箭頭跟方案說明欄（見 [Bug 紀錄](../testing/bug-log.md) BUG-021）；「方案總價」文字放在金額前面，實際金額由 `resolvePlanDisplayPrice` 計算（優先官方年繳總價，其次美金年繳定價透過 `useUsdToTwdRate` 即時匯率換算，而不是寫死用 `monthlyPrice×12` 概算）
...
- 「群組規則」：固定顯示 5 條輸入欄位，允許留白（不提供新增/移除），每條上限 80 字，送出時自動過濾空白項目
...
- 流程只有 4 步驟，不會另外切到成功頁：改用 `Modal`（`showHeader={false}`）疊在原本的步驟四表單頁上，顯示「群組已成功上架！」，並提供「返回首頁」與「前往群組管理」兩個按鈕
