...
## 使用技術
- **雖然分三步，UI 上其實是 2 個可捲動內容頁 + 1 個結果頁**：選擇方案跟篩選條件被合併在同一個可捲動容器裡，用左側常駐導覽加錨點捲動模擬子步驟切換
- **篩選條件跟結果都只存在頁面內的暫存狀態**：離開頁面就消失，不會被記錄下來
...
**4. 查看搜尋結果**
- 版面沿用步驟二「方案與條件」同一套左右分欄排版：左側是結果網格（可捲動），桌機版右側常駐顯示唯讀的條件摘要（不含移除服務按鈕），手機版收合成單欄、不顯示摘要；早期版本曾在頂部另外放一條橫式條件列，服務選多時會超出容器，已移除改用這個排版
- 結果網格前 3 名加上金/銀/銅名次徽章
- 每張卡片沿用探索頁的卡片樣式，點擊會開啟群組詳情（後續申請流程見 `apply-join-flow.md`），未登入時點申請會先導向登入頁
- 名次徽章畫在卡片內部本身的定位範圍內，不是外層另外疊一層蓋上去——這樣卡片 hover 放大時徽章才會跟著卡片一起縮放移動，不會維持原地不動看起來像脫落
- 卡片容器的捲動區留了足夠內距，讓最左/最右欄的卡片 hover 放大時不會被容器邊緣裁掉
