...
| `src/shared/ui/DisputeReasonDialog.jsx` | 唯讀對話框，顯示申訴人、申訴理由與附件連結；`MemberGroupView.jsx`／`src/features/manage-groups/components/HostGroupView.jsx` 都有掛載，讓成員與團主在群組進入 `disputed` 後都能點開查看同一份申訴內容，不用只靠聊天室文字傳達 |
