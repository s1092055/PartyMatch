...
若 `status` 不在允許清單中，回傳 `400 不允許從 X 轉換為 Y`。但實務上大部分轉換不是走這個泛用 PATCH，而是走下面列出的專屬端點（各自有自己的前置條件檢查，且部分轉換未出現在上表中，例如 `confirming → active` 實際是由 `/confirm` 端點處理，並非直接呼叫 `PATCH`）。
...
| `confirming → disputed` | 成員向平台申訴 | `POST /groups/:id/dispute` | 需為群組成員；狀態須為 `confirming`；`disputeDeadline = now + 3 天`；同一 transaction 寫入該成員 `serviceInfoIssueNote` / `disputeEvidenceUrl` |
| `disputed → active` | 平台客服裁定 | `POST /groups/:id/adjudicate`（`requireAdmin` 中介層，僅管理員） | 狀態須為 `disputed`；依 `winner` 分支：`member` 贏→退款給申訴成員、`member.delete`、該成員訂閱設 `cancelled`，群組 `escrowTokens` 扣除該席位費用後回 `active`；`host` 贏→全額撥款給團主、`escrowTokens` 歸零、全體訂閱轉 `active` |
