...
- **「填寫帳號」是群組概覽底部的動態按鈕**：跟「確認服務」「回報問題」一樣，只有在需要填寫帳號、或帳號被回報有問題時才會出現
- **不可逆操作要倒數確認**：確認服務、退出群組都要倒數幾秒才能真的送出，避免手滑誤觸
- **側邊欄固定項目**：符合退出條件（招募中或已額滿）時側邊欄底部固定顯示「退出群組」，跟「群組訊息」共用側邊欄右下角同一個位置
- **訂閱卡片（`SubscriptionCard.jsx`）依狀態切換統計格內容**：比團主端的 `HostedGroupCard.jsx`（見群組管理流程文件）簡單很多，中間格固定顯示「成員人數」，左格一律固定顯示「團主」（頂部 badge 已經顯示目前狀態，左格不重複顯示狀態文字），只有右格會依是否已啟用切換：

  | `sub.groupStatus`（判定用） | 卡片頂部 Badge | 左格 | 中格 | 右格 |
  |---|---|---|---|---|
  | `recruiting` 招募中 | 申請通過 | 團主：{團主名稱} | 成員人數 | 加入日期 |
  | `full` 額滿 | 等待鎖定（覆寫文字，跟團主端卡片一致） | 團主：{團主名稱} | 成員人數 | 加入日期 |
  | `pending_confirmation` 待成員填寫（自己已填完、等其他人） | 已填寫完成 | 團主：{團主名稱} | 成員人數 | 加入日期 |
  | `pending_confirmation` 待成員填寫（自己尚未填） | 成員填寫中 | 團主：{團主名稱} | 成員人數 | 加入日期 |
  | `pending_activation` 待啟用 | 待啟用 | 團主：{團主名稱} | 成員人數 | 加入日期 |
  | `confirming` 確認期中（自己尚未確認） | 確認期中 | 團主：{團主名稱} | 成員人數 | 加入日期 |
  | `confirming` 確認期中（自己已確認） | 服務中（視同已啟用） | 團主：{團主名稱} | 成員人數 | 下次扣款 |
  | `disputed` 申訴中 | 問題處理中 | 團主：{團主名稱} | 成員人數 | 加入日期 |
  | `active` 服務中 | 服務中（7 天內即將續訂時改顯示「即將續訂」） | 團主：{團主名稱} | 成員人數 | 下次扣款 |
  | `cancelled` 已解散 | 已解散 | 團主：{團主名稱} | 成員人數 | 加入日期 |
  | `ended` 已結束 | 已結束 | 團主：{團主名稱} | 成員人數 | 加入日期 |

  「已啟用」的判定不是單純看 `groupStatus === 'active'`：自己在確認期已經按下確認（`confirmedAt` 有值）的話，即使群組整體還沒等到其他成員確認、狀態上仍是 `confirming`，右格也會視同已啟用顯示「下次扣款」（因為對這位成員來說，該做的事已經做完了，比起「加入日期」更想知道下次扣款時間）
- **已申請分頁的卡片跟其他訂閱卡片視覺一致**：套用同樣的 hover 放大效果，狀態標籤改用共用徽章元件，不再手刻不同高度的標籤
...
- 頁面頂部會顯示「請填寫服務帳號以完成加入流程，剩餘 HH:MM:SS」倒數橫幅（鎖定時間起算 24 小時，每秒更新；逾期只顯示「已逾期」，不會有任何自動處理）
