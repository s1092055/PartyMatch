...
## 入口
- `/my-subscriptions`（獨立頁面，`SubscriptionsPage.jsx`）
...
- **不可逆操作要倒數確認**：確認服務、退出群組都要透過 `ConfirmActionDialog` 倒數幾秒才能真的送出，避免手滑誤觸
...
- 退出邏輯統一寫在 `src/features/group/utils/leaveGroupFlow.js` 的 `finalizeLeaveGroup`，`GroupDetailModal` 跟 `SubscriptionsPage.jsx` 兩個入口都呼叫同一份，避免各自維護一份重複邏輯、行為不一致
