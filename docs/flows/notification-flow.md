...
2. 前端每 5 秒定期輪詢通知列表，未讀數顯示在通知按鈕上；未登入使用者也能看到公開的系統公告
...
- **彈出提示（toast）跟通知中心分開設計**：toast 是即時、短暫的提醒，通知中心才是永久紀錄；提示分成「會影響使用者正在看的清單畫面」與「純即時、不影響任何畫面」兩種，前者顯示較久並帶操作按鈕、實際資料要等使用者按下按鈕才套用，避免正在瀏覽的清單內容被背景更新悄悄換掉；同一件事如果使用者自己的操作已經跳出更完整的錯誤訊息（例如鎖定群組時剛好有成員退出），就不會再重複跳一次語意相同的背景提示；分頁切到背景或離線一段時間後回來，累積的多筆提示會合併成一則，不會一次跳出一長串
