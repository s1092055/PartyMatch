...
### BUG-035：群組因某筆申請被接受而額滿時，同群組其他審核中的申請不會自動被拒絕或通知

- **功能**：[團主審核流程](../flows/approval-flow.md)，`server/src/utils/membership.js` 的 `advanceToFullIfNeeded`
- **嚴重度**：P1（不影響金流正確性，但使用者體驗上會誤以為系統壞掉——申請卡在審核中，代管費用也一直卡著沒退）
- **測試帳號**：見 `test-accounts.md`
- **操作步驟**：
  1. 兩位不同使用者對同一個群組送出申請（該群組剩 1 個名額）
  2. 團主接受其中一位申請，群組因此額滿（狀態轉為 `full`）
  3. 觀察另一位申請人的畫面/通知
- **預期結果**：另一位申請人應該立即收到通知，得知名額已滿、申請未通過，代管費用退還
- **實際結果**：另一位申請人的申請仍停在 `pending`，代管費用也還卡在群組的 `escrowTokens` 裡；只有等團主之後想到要手動點「拒絕」才會觸發退款與通知，在此之前系統完全沒有提示
- **推測原因**：`advanceToFullIfNeeded` 只把群組狀態推進到 `full`，沒有處理同群組其他還在 `pending` 的申請；額滿是被動觸發的（因為另一筆申請被接受），不會經過團主主動拒絕那條有退款+通知的路徑
- **修正狀態**：已修——新增 `autoRejectPendingApplications`，在群組轉為 `full` 的同一個 transaction 內，把同群組其他 `pending` 申請批次轉為 `rejected`、依各自 `escrowAmount` 退款，並各自發送「申請未通過（名額已滿）」通知
