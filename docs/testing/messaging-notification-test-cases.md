...
- 點擊「歡迎來到 PartyMatch」公告，會導向首頁 `/` 並整頁刷新（`window.location.replace('/')`），不是單純的 SPA 路由跳轉，也不是舊版的導向 `/explore`

---

### TC-215：訊息列表與群組成員面板的頭像狀態點

**前置條件**：兩個測試帳號 A、B 互為同一群組成員，且都已在「其他設定」設定過「目前狀態」（見 [`explore-account-test-cases.md`](./explore-account-test-cases.md) TC-436）。

**步驟**：
1. A 開啟訊息中心，查看與 B 的 DM 對話在列表中的頭像
2. A 開啟該群組聊天室，開啟成員面板，查看 B 的頭像
3. A 開啟跟「PartyMatch 系統訊息」的系統對話，查看該對話在列表中的頭像
4. B 切換「目前狀態」，A 重新整理或等待輪詢後再次查看上述兩處

**預期結果**：
- 步驟 1：DM 對話列表項目的頭像右下角顯示 B 目前狀態對應顏色的小圓點
- 步驟 2：群組成員面板裡 B 的頭像同樣顯示狀態點
- 步驟 3：系統對話沒有狀態點（沒有單一對應的真人使用者可比對，群組聊天室的多人共用頭像同理也不顯示）
- 步驟 4：B 變更狀態後，A 這邊的狀態點顏色應同步更新（輪詢間隔內）
...
### TC-210：申請人取消申請，團主收到通知並自動刷新申請列表

**前置條件**：某群組有一筆 `pending` 申請，團主與申請人各自登入不同瀏覽器/分頁，團主端「申請管理」分頁開著
**步驟**：
1. 申請人取消該筆 `pending` 申請（見 TC-102）
2. 團主端等待通知輪詢（不用手動整理頁面），觀察通知面板與「申請管理」分頁列表

**預期結果**：
- 團主應收到一筆 `application_cancelled` 通知（「申請人已取消申請」）
