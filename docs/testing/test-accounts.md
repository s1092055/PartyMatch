...
3. **獨立的管理員身分**（demo-admin，1 個）：申訴裁定（`POST /groups/:id/adjudicate`）與 `/admin` 管理員後台 Dashboard 只有 `isAdmin: true` 的帳號能操作，且應該跟一般團主/成員的日常操作分開，避免測試時混淆「這是用管理員身分做的，還是團主身分做的」
