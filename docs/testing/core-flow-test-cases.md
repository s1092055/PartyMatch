...
**前置條件**：以 demo4（張雅婷）登入，PM幣餘額（seed 完成後為 5571，建立群組本身不扣款，此步驟不影響餘額）。
...
**前置條件**：延續 TC-001 建立的群組（席位費用假設為 X PM，可從群組詳情頁確認）；以 demo5（李冠宇）登入，確認目前PM幣餘額（帳號中心可查看，seed 完成後為 3354）≥ X。

**步驟**：
1. 探索頁找到該群組，開啟群組詳情
2. 點擊申請加入，開啟 `ApplyModal` 彈窗（隱藏後方群組詳情視窗）
...
**前置條件**：以 demo6（黃詩涵，seed 完成後餘額為 305，刻意維持低額）登入；找一個席位費用高於餘額的群組（例如 seed 資料中的 G6 ChatGPT Team，每人 800 PM；或 G13 Cursor，每人 1707 PM，或自建一個高單價群組）。
...
### TC-012：App 啟動時部分資料載入失敗跳出彙總 Toast

**前置條件**：可暫時關閉後端伺服器，或用瀏覽器開發工具攔截其中一個 store 的初始化請求（例如 `GET /api/groups`）使其失敗
**步驟**：
1. 重新整理頁面，讓 `App.jsx` 的 `bootApp()` 重新跑一次初始化
2. 觀察畫面跳出的 Toast 數量與內容
...
**前置條件**：可用瀏覽器開發工具攔截並讓其中一個寫入請求回傳失敗（例如收藏、成員名單、訂閱、群組更新、通知已讀等任一有樂觀更新的 action）
**步驟**：
1. 觸發一個樂觀更新的 action（例如加入收藏、標記通知已讀）
2. 讓對應的後端請求回傳失敗（4xx/5xx 或斷網）
3. 觀察畫面狀態與 Toast
