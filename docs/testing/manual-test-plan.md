...
npm run db:seed-demo    # demo 帳號、群組、申請、通知、對話
```
...
2. **探索群組**：分類篩選、關鍵字搜尋、價格上限、排序、URL query params 狀態
3. **快速搜尋**（`/quick-match`）：免登入使用、選服務/方案/條件、配對結果
4. **建立群組**（`/create-group`）：4 步驟表單（選服務 → 選方案 → 群組設定 → 確認送出）
5. **申請加入群組**：餘額檢查、留言、同意條款、撤回申請
6. **團主審核申請**：核准（扣款、建群、額滿推進）、拒絕（可重新申請）
...
9. **填寫服務帳號資訊**：成員填寫、全員完成後自動推進
10. **啟用服務**：團主啟用、48h 確認期、主動確認、逾期自動撥款
11. **爭議申訴**：成員申訴（含附件上傳）、平台裁定（需管理員）
...
18. **我的群組頁**：成員/團主視角切換、統計卡、篩選分頁、群組紀錄（已結束/已取消）
...
- 建立群組 → 申請加入 → 審核核准（扣款）→ 名額額滿 → 鎖定群組 → 填寫服務帳號資訊 → 全員完成自動推進 → 團主啟用服務 → 確認期確認/逾期自動撥款
- 申請時PM幣餘額不足的擋下邏輯
- 名額已滿時無法申請
- 團主審核拒絕、成員撤回申請
