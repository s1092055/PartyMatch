...
`winner` 只接受 `member`／`host`，二選一，其餘回 `400`；沒有部分退款選項。`winner: member` 退還申訴成員的席位費用；`winner: host` 該筆費用不予退還。兩種結果都**不會**把申訴成員移出群組，只有金流方向不同。團主撥款金額一律由伺服器以「代管總額 − 成員退款」算出，不採信前端數字。裁定結果會寫入獨立的申訴紀錄，管理員後台可查詢每筆申訴從提出到結案的完整歷史（含申訴內容、佐證、雙方對話記錄），皆包在單一 Prisma transaction 內，並用條件式搶佔避免跟團主同時自行協調解決（`resolve-dispute`）互相衝突。
