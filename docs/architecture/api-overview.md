...
除了 `GET /health`（健康檢查，不掛 `/api` 前綴，回傳 `{ status: 'ok', ts }`）以外，其餘端點均掛載於 `/api` 前綴下（見 `server/src/app.js`）。認證欄位標示：`公開` 無需 token；`optionalAuth` 有 token 則帶出更多資料，無 token 也可呼叫；`需登入` 必須帶合法 accessToken；`需管理員` 需 `isAdmin: true`。完整 request/response schema 請直接參考對應 route 檔案內的 `zod` schema。
...
| Method | Path | 認證 | 說明 |
|--------|------|------|------|
| GET | `/groups` | optionalAuth | 探索群組列表；query 參數 `serviceId`／`category`／`q`／`status`（預設 `recruiting`，帶 `status=all` 才回全部狀態——這個預設值是前端兩階段初始化 `init({ all: false })`／`init({ all: true })` 的依據） |
| GET | `/groups/:id` | optionalAuth | 群組詳情；讀取當下若群組是 `confirming` 且 `confirmDeadline` 已過，會惰性觸發自動撥款 |
| POST | `/groups` | 需登入 | 建立群組 |
| PATCH | `/groups/:id` | 需登入 | zod 只收 `status`／`billingCycle`／`nextBillingDate` 三個欄位，不是任意群組設定；`status` 只能照 `ALLOWED_TRANSITIONS` 白名單轉換 |
| POST | `/groups/:id/activate` | 需登入 | `pending_activation → confirming`，開始 48h 確認期 |
| POST | `/groups/:id/confirm` | 需登入 | 成員確認服務正常，回傳 `{ group, released }`，`released` 供前端判斷是否已撥款 |
| POST | `/groups/:id/dispute` | 需登入 | 成員申訴（`confirming → disputed`） |
| POST | `/groups/:id/cancel` | 需登入 | 解散群組（啟用前），退還所有代管 PM 幣 |
| POST | `/groups/:id/lock` | 需登入 | `full → pending_confirmation`；body `{ sharedCredentials }`（`shared_credentials` 分類的服務用來存團主填的結構化帳密） |
| POST | `/groups/:id/adjudicate` | 需管理員 | 裁定申訴，`winner: 'member' \| 'host'` |
| POST | `/groups/:id/renew` | 需登入 | `active → pending_confirmation`，向每位成員收取本期代管費用；餘額不足回 400 `INSUFFICIENT_BALANCE`，條件式扣款競爭失敗回 409 |
...
| POST | `/members` | 需登入 | 團主手動加入成員；僅 `recruiting` 狀態可用，會直接從被加入者的 `tokenBalance` 扣代管席位費 |
| PATCH | `/members/:id` | 需登入 | 成員本人或團主更新（如填寫訂閱帳號資訊） |
| DELETE | `/members/:id` | 需登入 | 團主移除成員，或成員本人退出；僅 `recruiting`／`full` 狀態可用，之後回 400 |
...
| PATCH | `/subscriptions/:id` | 需登入 | 更新 `status`（`pending`/`active`/`ended`）／`nextBillingDate`；PM幣代管取代了原本的「標記付款/確認付款」概念，這裡不處理金流 |
...
| Method | Path | 認證 | 說明 |
|--------|------|------|------|
| GET | `/tokens` | 需登入 | 目前 PM 幣餘額與最近 50 筆交易；帶 `?groupId=` 改回該群組的完整交易紀錄（不受 50 筆上限），成員視角付款面板用 |
...
| Method | Path | 認證 | 說明 |
|--------|------|------|------|
| POST | `/upload/dispute-evidence` | 需登入 | 上傳申訴佐證檔案（png/jpeg/gif/webp/heic/pdf），存進 Cloudflare R2，回傳 `{ url, publicId }` |
