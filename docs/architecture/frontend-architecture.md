...
10 個 Zustand store，每個對應一個後端 REST 資源，採「記憶體快取」模式（`init()` 讀取、`create`/`update` 呼叫 API 後更新本地快取）。選擇 Zustand 而非 Redux／純 Context，主要考量是這裡的狀態本質是「伺服器資料的本地快取」，不需要 Redux 的完整可追蹤樣板碼，也需要比 Context 更細緻的訂閱粒度以避免不必要的重渲染。

App 啟動時分兩階段初始化 store：先載入不需登入的公開資料（服務清單、招募中群組、通知等），登入後才載入私人資料（申請、訂閱、成員、收藏、對話），避免未登入狀態呼叫受保護端點。
...
四大類路由：公開路由（首頁、探索、登入/註冊等）、需登入路由（巢狀於帶 sidebar/dock 的版面內，由路由守衛保護）、獨立的全螢幕步驟流程頁（建立群組、快速配對），以及不對外公開入口的管理員後台（獨立版面與路由守衛，非管理員一律導回首頁，不顯示登入提示）。

導覽列依裝置寬度切換成桌機 sidebar 與手機 header/dock 兩套完全不同的 UI 元件，斷點選在 1280px（而非 768px）以避免觸控裝置被誤判為桌機。
