...
- **手機與平板版**（<1280px，含 iPad）：頂部 `MobileHeader.jsx` 用 flex 排版（logo bar `flex-1` + 圓形按鈕 `shrink-0`，不手動湊 `right-[Nrem]` 這種跟按鈕尺寸綁死的數字），只留 Logo，右側依登入狀態顯示通知（訪客也看得到）／訊息（僅登入）兩顆獨立圓形按鈕；底部 `MobileDock.jsx`（快速搜尋、建立群組、探索置中圓形按鈕、我的、我的帳號／登入），往下捲動時 Dock 會滑出隱藏、往上捲或接近頁面頂端時顯示。「我的」（群組管理／我的訂閱／我的收藏，項目直接讀 `nav.js` 的 `NAV_SECTIONS`，跟桌面版 sidebar 同一份資料）跟「帳號選單」都改用共用 `Drawer` 元件（`components/ui/drawer.jsx`，Base UI）做成從螢幕底部滑出的 bottom sheet，不用貼在觸發點上方的小 popup——header 貼在螢幕最上緣，往上展開的傳統 dropdown 會被螢幕邊界裁掉大半，且 Drawer 本身的背景點擊／Esc 就會處理「點外面關閉」，不需要額外用 `useClickOutside`。帳號選單登入時顯示大頭像／名稱／PM幣／我的帳號／登出，未登入時顯示 PartyMatch logo＋「訪客」、只留登入入口；深色模式切換兩種身份都能用，不強制要求登入。

未登入時，「建立群組」「我的」仍會顯示鎖頭圖示，點擊觸發 `preventLockedAction`（提示前往登入，不直接導頁）；「我的帳號」（手機版）／登入按鈕不受此限，一律可點開帳號選單或直接前往登入頁。
