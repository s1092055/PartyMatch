...
- 探索共享訂閱群組（分類篩選、價格與排序條件）
- 快速搜尋：輸入服務、方案、預算，系統推薦最符合的群組
...
- **獨立步驟流程頁**：建立群組、快速搜尋為多步驟全螢幕頁面（非 Modal），脫離主要導覽（sidebar / Dock）避免視覺干擾；兩者皆以 `key={step}` 重新掛載搭配 CSS slide-up 動畫切換步驟，並透過共用的 `useScrollEdge` hook（`ResizeObserver` + 可選 `MutationObserver`）動態偵測內容是否溢出，僅在確定不會裁切內容時才置中顯示，可捲動時則顯示共用的 `ScrollHintButton` 提示按鈕
