...
- 收藏感興趣的群組
- 使用聊天室與團主或其他成員溝通（群組聊天室 + 私人 DM，見 `src/features/messages/`）
- 管理個人資料、付款方式與PM幣餘額
...
- **獨立步驟流程頁**：建立群組、快速搜尋為多步驟全螢幕頁面（非 Modal），脫離主要導覽（sidebar / Dock）避免視覺干擾；兩者皆以 `key={step}` 重新掛載搭配 CSS slide-up 動畫切換步驟，共用頁首左上角的 PartyMatch logo（一般 `<a>` 超連結，點擊回首頁並觸發整頁重新載入）；快速搜尋的內容高度會依 `useScrollEdge` 動態判斷是否置中，建立群組的方案設定步驟則固定頂部對齊，避免表單內容變動（如新增群組規則）造成置中與否切換時的畫面跳動
